...
#if defined(NDEBUG)
# error "Weisscoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xac02d7f03a7edd688cd76e3d84cab4a8db3c4117da7f8c30bc1110781d8dbf27");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Weisscoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Weisscoin Signed Message:\n";
...
{
    // Weisscoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 20000); // Weisscoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Weisscoin: 1 days
static const int64 nTargetSpacing = 1 * 60; // Weisscoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Weisscoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Weisscoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x606774a800b5bc27128f0ba2d978310613f29d7b75c6c2d1b4dd0594dc72237b");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Weiss it the best man";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 20 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04f1bd566204d228ce4522b37f1b1f672828b9dbe1836ad02ae9c6f212b7af1627729d03f6c934cc5df413d4e5bd9ad0eff3da3a977bd3c416cd00488b2ab501ad") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1540206485;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2085586439;

        if (fTestNet)
        {
            block.nTime    = 1540206461;
            block.nNonce   = 388209810;
        }
	...
	if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0xee1331575f9f8eb11cfd9c716c19ca866632b43f282da72c5f3acd196c7c02c4"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfe, 0xc3, 0xba, 0xd3 }; // Weisscoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("WeisscoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("WeisscoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static WeisscoinMiner(CWallet *pwallet)
{
    printf("WeisscoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("weisscoin-miner");
    ...
    try { loop {
        ...
        printf("Running WeisscoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("WeisscoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&WeisscoinMiner, pwallet));
}
